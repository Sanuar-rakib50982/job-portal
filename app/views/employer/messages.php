...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($action === "send") {
        ...
        if ($recipientId <= 0) {
            ...
            if ($employer->sendMessage($employerId, $recipientId, $applicationId, $body)) {
    ...
    if ($action === "reply") {
        ...
        if ($messageId > 0) {
            $employer->markMessageAsRead($messageId, $employerId);
        }
    ...
    if ($action === "mark_read") {
        ...
        if ($messageId > 0 && $employer->markMessageAsRead($messageId, $employerId)) {
...
$seekers = $employer->getSeekersForMessage();
$applicationsForMessage = $employer->getEmployerApplicationsForMessage($employerId);
$receivedMessages = $employer->getReceivedMessages($employerId);
$sentMessages = $employer->getSentMessages($employerId);
?>
        ...
        <div class="table-box">
            ...
            <?php if ($receivedMessages->num_rows > 0) { ?>
                <?php while ($msg = $receivedMessages->fetch_assoc()) { ?>
                    <div class="job-card">
                        <h3>Message from <?php echo htmlspecialchars($msg['sender_name']); ?></h3>
        ...
        <div class="table-box">
            ...
            <?php if ($sentMessages->num_rows > 0) { ?>
                <?php while ($sent = $sentMessages->fetch_assoc()) { ?>
                    <div class="job-card">
                        <h3>Message to <?php echo htmlspecialchars($sent['recipient_name']); ?></h3>
                        ...
                        <p><?php echo nl2br(htmlspecialchars($sent['body'])); ?></p>
                    </div>
                <?php } ?>
            <?php } else { ?>
